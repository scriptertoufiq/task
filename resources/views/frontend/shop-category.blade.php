                ...
                <div class="col-lg-3">
                    <div class="sidebar-widget">
                        <div class="single-sidebar-widget">
                            ...
                            <ul class="course-categoris">
                                @if ($cart->where('id', $shopproduct->id)->count())
                                    ...
                                    {{-- {{ Gloudemans\Shoppingcart\Facades\Cart::weight() }} --}}
                            </ul>
                            ...
                            <div style="background: #175689;color: #ffffff; padding: 10px 2px;"> <button type="submit"
                                    class="btn btn-success">Checkout</button> </div>
                            @endif
                        </div>
                        <div class="single-sidebar-widget">
                        ...
                        <div class="single-sidebar-widget">
                            ...
                            <ul class="tags">
                                @foreach ($tags as $tag)
                                    <li><a href="{{ url('shop/tag/'.$tag->id) }}"> {{ $tag->name }} </a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="POST" action="{{ url('/cart/store') }}"
                        ...
                        <select name="orderSize" id="orderSize" style="flex-grow:0;">
                            <option selected disabled>Select Size</option>
                            @foreach ($shopproductsize as $shopproductsize)
                                <option value="{{ $shopproductsize->id }}">{{ $shopproductsize->size }}</option>
                            @endforeach
                        </select>
                        <button class="btn btn-success ml-2" disabled id="cartAddBtn" style="align-self:flex-end"
                            type="submit"><i class="fa fa-cart-plus"></i> Add to cart</button>
                    </form>
                </div>
            </div>
        </div>
...
@section('script')
    <script>
        $(".viewProductBtn").click(function(e) {
            ...
            $("#orderSize").change(function(e) {
                e.preventDefault();
                $("#cartAddBtn").removeAttr('disabled');
            });
        });
